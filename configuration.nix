...
in
{
  ...
  environment.etc."polyflow/metadata.json.age" =
    builtins.trace "[BUILD] Checking metadata.json.age at build time: ${toString (builtins.pathExists ./metadata.json.age)}"
    (lib.mkIf (builtins.pathExists ./metadata.json.age) {
      source = ./metadata.json.age;
      mode = "0400";  # Read-only for root
    });

  environment.etc."polyflow/sops-key.txt" =
    builtins.trace "[BUILD] Checking sops-key.txt at build time: ${toString (builtins.pathExists ./sops-key.txt)}"
    (lib.mkIf (builtins.pathExists ./sops-key.txt) {
      source = ./sops-key.txt;
      mode = "0400";  # Read-only for root
    });
